...
exports.getEvents = async (req, res) => {
  try {
    ...
    for (const event of events) {
      ...
      // 🔥 Calcul publicStatus
      // 🔥 Calcul publicStatus
      // Default to actual event status (PENDING, REJECTED, DRAFT)
      let publicStatus = event.status;

      // Only calculate time-based status for PUBLISHED events
      if (event.status === 'PUBLISHED') {
        const now = new Date();
        if (now > event.endAt) {
          publicStatus = "ENDED";
        } else if (now >= event.startAt && now <= event.endAt) {
          publicStatus = "ONGOING"; // treated as Published/Upcoming in frontend
        } else {
          publicStatus = "UPCOMING"; // treated as Published in frontend
        }
      }
...
exports.createEvent = async (req, res) => {
  try {
    ...
    // Determine status (default PENDING, allow DRAFT)
    ...
    const event = await Event.create({
      ...
      coverImageUrl: req.file ? req.file.path : req.body.coverImageUrl, // Prioritize uploaded file
...
exports.updateEvent = async (req, res) => {
  try {
    ...
    // 🔄 Dacă evenimentul era RESPIN sau CIORNĂ și se cere PENDING, actualizăm
    // Also allow direct status update if passed in body (e.g. from Draft -> Pending)
    if (req.body.status && ['PENDING', 'DRAFT'].includes(req.body.status)) {
      event.status = req.body.status;
    } else if (event.status === 'REJECTED') {
      // Auto-reset REJECTED to PENDING on edit if not explicitly set
